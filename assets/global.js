  ...
  class VariantSelects extends HTMLElement {
	...
	renderProductInfo() {
	  fetch(`${this.dataset.url}?variant=${this.currentVariant.id}&section_id=${this.dataset.section}`)
		...
		.then((responseText) => {
		  const id = `productTemplate-${this.dataset.section}`;
		  ...
		  const productTemplate = document.getElementById(`productTemplate-${this.dataset.section}`);
  
		  if (productTemplate) productTemplate.classList.remove('visibility-hidden');
		  this.toggleAddButton(!this.currentVariant.available, window.variantStrings.soldOut);
		});

		fetch(`${this.dataset.url}?variant=${this.currentVariant.id}&shopify-section-${this.dataset.section}`)
		.then((response) => response.text())
		.then((responseText) => {
			const id = 'product-details-accordion';
			const html = new DOMParser().parseFromString(responseText, 'text/html')
			const destination = document.getElementById(id);
			const source = html.getElementById(id);
	...
	toggleAddButton(disable = true, text, modifyClass = true) {
	  ...
	  const addButtonText = productForm.querySelector('[name="add"] .button__text');
